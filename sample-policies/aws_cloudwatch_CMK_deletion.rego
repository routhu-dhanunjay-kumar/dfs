...
filter_has_matching_alarm(filter) {
    // Check that the metric filter has a filter pattern that matches CMK deletion or disabling.
    filter_pattern := filter.filter_pattern
    contains(filter_pattern, "DisableKey")
    contains(filter_pattern, "ScheduleKeyDeletion")
