...
filter_has_matching_alarm(filter) {
    // Check that the metric filter has a filter pattern that matches usage of the AWS root account.
    filter_pattern := filter.filter_pattern
    contains(filter_pattern, "\"AuthenticatedIdentity\": {\"principalId\": \"AWS:*\"}")
